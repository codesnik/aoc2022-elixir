...
count_reverse = fn row, prev_count ->
  ...
  |> case do {_, count, reversed_row} -> {reversed_row, count} end
end

File.stream!("input.txt")
|> Stream.map(&String.trim/1)
|> Enum.map(fn line -> Enum.map(String.to_charlist(line), &{&1, false}) end)
|> case do rows -> {rows, 0} end
|> case do {rows, count} -> Enum.map_reduce(rows, count, count_reverse) end
|> case do {rows, count} -> Enum.map_reduce(rows, count, count_reverse) end
|> case do {rows, count} -> {Enum.zip_with(rows, &(&1)), count} end
|> case do {rows, count} -> Enum.map_reduce(rows, count, count_reverse) end
|> case do {rows, count} -> Enum.map_reduce(rows, count, count_reverse) end
|> case do {_rows, count} -> count end
|> IO.puts
