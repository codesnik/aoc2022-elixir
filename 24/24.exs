...
defmodule X do
  ...
  def wander(map, max_x, max_y, mes, theend, n) do
    ...
    if theend in mes do
      ...
      wander(map, max_x, max_y, mes, theend, n + 1)
    end
  end
  ...
  def display(map, max_x, max_y, mes) do
    for y <- 0..(max_y - 1) do
      for x <- 0..(max_x - 1) do
        if {x, y} in mes && map[{x, y}], do: raise("wtf, #{x} #{y} #{map[{x, y}]}")
  ...
  def start(mapstr) do
    ...
    wander(map, max_x, max_y, [], {max_x - 1, max_y - 1}, 0) + 1
  end
end
